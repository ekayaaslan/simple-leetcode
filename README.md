...
<b>Linear search:</b> Check every element in an array one by one until the target is found.
...
<b>Brute force:</b> A straightforward approach that exhausts every possible solution to find the correct one, guaranteeing a result.
...
<b>Direct addressing:</b> Using the value of an item directly as an index in an array or lookup table for constant access.
...
<b>Canonicalising:</b> Transforming data into a "standard" format so that equivalent items look identical.
...
<b>Two pointers:</b> A technique using two indices to traverse a data structure, typically moving towards each other or in parallel.
